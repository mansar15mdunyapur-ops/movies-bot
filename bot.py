...
# Configuration
BOT_TOKEN = "8516339596:AAEeiZCTS6AEqvOiR7oBFxPEpdHLliKKjKw"
TMDB_API_KEY = "2e822b46b04411667ee5a3723c78e674"
...
def restricted(func):
    ...
    async def wrapped(update, context, *args, **kwargs):
        if not update.effective_user:
            return
        has_access, status = payment.check_user_access(update.effective_user.id)
        if not has_access and status != "new":
            keyboard = [[InlineKeyboardButton("💳 Buy Subscription", callback_data="show_payment")]]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_msg = (
        ...
        "Main aapka **Advanced Movie Bot** hoon!\n\n"
        "**✨ Features:**\n"
        "🎥 Movie Search - TMDb se details\n"
        "📥 Direct Download - Agar movie available hai\n"
        "ℹ️ Movie Info - Rating, cast, story\n"
        "📝 Request System - Jo movie nahi, vo request karo\n\n"
        "**📌 Commands:**\n"
        "/movies - Humari collection dekhein\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        "📚 **Bot Guide** 📚\n\n"
        "**🎯 Movie Search:**\n"
        "• Direct naam likho - TMDb se results\n"
        "• Agar humari collection mein hai to file milegi\n\n"
        "**📋 Commands:**\n"
        ...
        "/help - Yeh message\n\n"
        "**📥 Download:**\n"
        "Movie select karo → Quality choose karo → File mil jayegi\n\n"
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check access for non-payment buttons
    if not query.data.startswith(('buy_', 'paid_', 'show_payment')):
        has_access, status = payment.check_user_access(query.from_user.id)
        if not has_access and status != "new":
            keyboard = [[InlineKeyboardButton("💳 Buy Subscription", callback_data="show_payment")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    if action == "download":
        tmdb_id = int(data[1])
        movie_data = database.get_movie_by_tmdb(tmdb_id)
        
        if movie_data and movie_data['qualities']:
            # Multiple qualities hain to choose karo
            if len(movie_data['qualities']) > 1:
                keyboard = []
                for quality, file_id, file_size in movie_data['qualities']:
                    btn_text = f"{quality} ({file_size})"
                    keyboard.append([InlineKeyboardButton(btn_text, callback_data=f"file_{file_id}_{quality}")])
                ...
                # Ek hi quality hai to direct send karo
                quality, file_id, file_size = movie_data['qualities'][0]
                await query.message.delete()
                await query.message.reply_video(
                    video=file_id,
                    caption=f"🎬 {movie_data['movie'][2]} ({movie_data['movie'][3]})\n📥 Quality: {quality}\n💾 Size: {file_size}"
                )
        else:
            await query.edit_message_text("❌ Movie file nahi mili!")
    
    # Send actual file
    elif action == "file":
        file_id = data[1]
        quality = '_'.join(data[2:])  # Join remaining parts for quality
        await query.message.delete()
        await query.message.reply_video(
            video=file_id,
...
def main():
    ...
    # Start bot
    print("🎬 Advanced Movie Bot starting...")
